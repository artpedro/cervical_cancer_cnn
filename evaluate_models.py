...
def main():
    ...
    for target_name, tgt in dataset_map.items():
        ...
        for ckpt_path in tqdm(model_files, desc=f"Validating on {target_name}"):
            try:
                if os.path.exists(MODELS_DIR / "all_cross_results" / "cross_dataset_validation_results.csv"):
                    if ckpt_path.relative_to(MODELS_DIR) in evaluated_paths:
                        print(f"✅  Already evaluated: {ckpt_path.relative_to(MODELS_DIR)}")
                        continue
                print(ckpt_path)
                
                # change \ on paths to \\
                ckpt_path = ckpt_path.resolve().as_posix()
...
if __name__ == "__main__":
    main()
>>>>>>> db513a6 (Refac: folder organization)
