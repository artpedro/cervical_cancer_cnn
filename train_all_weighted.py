...
def main():
    ...
    print("="*70)
    print("All datasets will use WeightedRandomSampler for balanced batches.")
    print("Loss function: Standard CrossEntropyLoss (no class weights)")
    print("="*70 + "\n")
    ...
    for root, scanner, name in zip(roots, scanners, names):
        ...
        total = len(df)
        print(f"\nClass Distribution:")
        print(f"  Class 0 (Normal):   {class_dist[0]:>5} samples ({100*class_dist[0]/total:>5.1f}%)")
        if 1 in class_dist.index:
            print(f"  Class 1 (Abnormal): {class_dist[1]:>5} samples ({100*class_dist[1]/total:>5.1f}%)")
            imbalance_ratio = max(class_dist[0], class_dist[1]) / min(class_dist[0], class_dist[1])
            print(f"  Imbalance Ratio: {imbalance_ratio:.2f}:1")
        print(f"\n→ WeightedRandomSampler will balance batches during training.\n")
    ...
    print("Compare with original (class weights) results to see the difference!")
    print("="*70 + "\n")
