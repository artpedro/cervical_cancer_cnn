...
# ----------------------------
#
# Model saving (per cross-dataset plan):
#   - run_dir = METRICS_DIR / dataset_name / balance_mode / "%Y-%m-%d_%H-%M-%S"
#   - Checkpoints: run_dir / "checkpoints" / {name}_{backbone_id}_{balance_mode}_best_fold{fold}.pt
#   - Content: model.state_dict() only (no optimizer, epoch, or config in .pt)
#   - Mapping: training_time_results.csv and summary_*.csv provide (dataset, model, fold, best_epoch, run_dir)
    ...
    *,
    ...
    for friendly_name, backbone_id in models.items():
        for fold in range(num_folds):
            ...
            # --------- transforms (per-fold from JSON to avoid leakage) ---------
            if stats_path is not None:
                _train_tf, _val_tf = make_tf_from_stats_for_fold(name, fold, stats_path)
            else:
                _train_tf, _val_tf = train_tf, val_tf
            if _train_tf is None or _val_tf is None:
                raise ValueError("Provide either stats_path or both train_tf and val_tf")
